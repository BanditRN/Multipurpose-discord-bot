...
var { databasing, isValidURL, delay } = require(`${process.cwd()}/handlers/functions`);
const fs = require("fs");
module.exports = {
    name: "reloadbot",
    category: "👑 Owner",
...
\`\`\``
      ...
      `,
            ...
            await client.socialhandlers.forEach(handler => {
                try {
                    delete require.cache[require.resolve(`../../social_log/${handler}`)];
                    console.log(`SUCCESS :: ../../social_log/${handler}`);
                } catch (e) {
                    console.log(e.stack ? String(e.stack).dim : String(e).dim);
                }
            });
