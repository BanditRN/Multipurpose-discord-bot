...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
        ...
        try {
            ...
            const collector = awaitedmsg.createMessageComponentCollector({
                ...
                if (b?.customId == "1") {
                    ...
                    await asyncForEach(Users, async user => {
                        try {
                            ...
                            let memberData = await client.invitesdb?.find(
                                v => (v.id == user.id && v.guildId == message.guild.id && v.bot == user.bot) || false
                            );
                            let memberDataKey = await client.invitesdb?.findKey(
                                v => (v.id == user.id && v.guildId == message.guild.id && v.bot == user.bot) || false
                            );
                            memberData.invites = guildInvites
                                .filter(i => i?.inviter.id === user.id)
                                .map(i => i?.uses)
                                .reduce((p, c) => p + c);
