            ...
            if ((users && users.length > 0) || (roles && roles.length > 0)) {
                ...
                message.reply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setFooter(client.getFooter(es))
                            .setTitle(
                                `${allEmojis.msg.SUCCESS} **Successfully unlocked \`${channel.name}\` for ${users ? users.length : 0} Users and ${roles ? roles.length : 0} Roles**`
                            ),
                    ],
                });
            } else {
                if (
                    channel.permissionOverwrites.cache.filter(permission =>
                        permission.deny.toArray().includes("SEND_MESSAGES")
                    ).size < 1
                )
                    return message.reply({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    `This usually means, that the Channel **PERMISSIONS** are so defined, that __all__ of them are ALLOWING to send a Message!`
                                ),
                        ],
                    });
