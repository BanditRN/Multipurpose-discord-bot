...
> This will send the winner(s) of the Giveaway, received from the Database`),
        ...
        if (args[0].toLowerCase() === "start") {
            try {
                ...
                var collected = await message.channel.awaitMessages({
                    filter: m => m.author.id == originalowner,
                    max: 1,
                    time: 60e3,
                    errors: ["time"],
                });
                ...
                let options = {
                    ...
                    thumbnail: es.thumb
                        ...
                        : null,
                    botsCanWin: false,
                    embedColor: require("discord.js").Util.resolveColor(es.color)
                        ? require("discord.js").Util.resolveColor(es.color)
                        : 3932049,
                    embedColorEnd: require("discord.js").Util.resolveColor(es.wrongcolor)
                        ? require("discord.js").Util.resolveColor(es.wrongcolor)
                        : 16731451,
                ...
                if (bonusentriesdata.mentions.roles.size > 0) {
                    ...
                    if (bonusentriesdata.mentions.roles.size > 1) {
                        ...
                        [...bonusentriesdata.mentions.roles.values()].forEach((role, index) => {
                            ...
                            options.bonusEntries.push({
                                ...
                                bonus: new Function(
                                    "member",
                                    `return member && member.roles && member.roles.cache.size > 1 && member.roles.cache.some((r) => r.id === '${roleid}') ? '${Amount}' ? '${Amount}' : 1 : null`
                                ),
                                cumulative: true,
                            });
                    ...
                    //One Bonus entrie
                        ...
                        options.bonusEntries.push({
                            ...
                            bonus: new Function(
                                "member",
                                `return member && member.roles && member.roles.cache.size > 1 && member.roles.cache.some((r) => r.id === '${bonusentriesdata.mentions.roles.first().id}') ? Math.floor(Number('${bonusentriesdata.content.split(" ")[1]}')) ? Math.floor(Number('${bonusentriesdata.content.split(" ")[1]}')) : 1 : null`
                            ),
                            cumulative: true,
                        });
                ...
                if (requiredroles.mentions.roles.size >= 1) {
                    ...
                    options.exemptMembers = new Function(
                        "member",
                        `return !member || !member.roles ||!member.roles.cache.some((r) => '${theRoles}'.includes(r.id))`
                    );
                }
            ...
            client.giveawaysManager
                .reroll(giveaway.messageId, { winnerCount: !isNaN(args[1]) ? Number(args[1]) : 1 })
                .then(() => {
                    message.reply({
                        content:
            ...
            if (args[0] && args[0].toLowerCase() === "server") {
                ...
                return swap_pages(
                    ...
                    message,
                    buffer,
                    eval(client.la[ls]["cmds"]["administration"]["giveaway"]["variable45"])
                );
            }
            ...
            for (let i = 0; i < allGiveaways.length; i++) {
                try {
                    let invite =
                        client.guilds.cache.get(allGiveaways[i].guildId).invites.cache.size > 0
                            ? client.guilds.cache.get(allGiveaways[i].guildId).invites.cache.map(invite => invite.url)[0]
                            : client.guilds.cache
            ...
            return swap_pages(
                ...
                message,
                buffer,
                eval(client.la[ls]["cmds"]["administration"]["giveaway"]["variable46"])
            );
            ...
            if (giveaway.messages && giveaway.messages.winMessage && giveaway.messages.winMessage.includes("{winners}")) {
                return message.reply({
                    content: `${giveaway.messages.winMessage
                        .replace("{winners}", giveaway.winnerIds.map(d => `<@${d}>`).join(", "))
                        .replace("{this.prize}", giveaway.prize)
                        .replace(
                            "{this.messageURL}",
