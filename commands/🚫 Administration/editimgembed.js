...
module.exports = {
    ...
    description:
        'DONT FORGET TO ADD THE "++"! They are needed, and used to declare where the TITLE and where the DESCRIPTION is!\nEdits an already existing Embed',
    type: "server",
    run: async (client, message, args, cmduser, text, prefix) => {
        ...
        try {
            ...
            if (client.settings.get(message.author.id, "dm"))
                message.author
                    .send(`${ee}\`\`\`${message.content}`.substring(0, 2040) + "```")
            ...
            message.channel.messages
                ...
                .then(msg => {
                    msg.edit({
                        ...
                        if (client.settings.get(message.author.id, "dm"))
                            message.author
                                .send({ content: `${ee}\`\`\`${message.content}`.substring(0, 2040) + "```" })
                                .catch(e => console.log("Couldn't Dm Him this log prevents a crash"));
                    });
                })
                .catch(e => {
                    return message.reply({
                        content: `${e.message ? String(e.message).substring(0, 1900) : String(e).grey.substring(0, 1900)}`,
                        code: "js",
                    });
                });
