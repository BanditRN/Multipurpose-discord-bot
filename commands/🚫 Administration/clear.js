...
const { delay, databasing } = require(`${process.cwd()}/handlers/functions`);
module.exports = {
    name: `clear`,
    aliases: [`purge`],
                ...
                if (args[1].toLowerCase() == "bots" || args[1].toLowerCase() == "bot") {
                    ...
                    if (args[0]) {
                        if (Number(args[0]) > 5000 || Number(args[0]) < 0)
                            return message.reply({
                                content: eval(client.la[ls]["cmds"]["administration"]["clear"]["variable4"]),
                            });
                        if (isNaN(args[0]))
                            return message.reply({
                                content: eval(client.la[ls]["cmds"]["administration"]["clear"]["variable5"]),
                            });
                    ...
                    for (let i = 0; i < msgs.length; i += 100) await message.channel.bulkDelete(msgs.slice(i, i + 100));
                    ...
                    if (!user)
                        return message.reply({
                            content: eval(client.la[ls]["cmds"]["administration"]["clear"]["variable8"]),
                        });
                    let messageCollection = new Collection(); //make a new collection
                    let channelMessages = await message.channel.messages
                        .fetch({
                    ...
                    if (args[0]) {
                        if (Number(args[0]) > 5000 || Number(args[0]) < 0)
                            return message.reply({
                                content: eval(client.la[ls]["cmds"]["administration"]["clear"]["variable9"]),
                            });
                        if (isNaN(args[0]))
                            return message.reply({
                                content: eval(client.la[ls]["cmds"]["administration"]["clear"]["variable10"]),
                            });
                        messagelimit = Number(args[0]) / 100;
                    }
                    if (Number(args[0]) > 100) {
                        while (channelMessages.size === 100) {
                            ...
                            if (channelMessages)
                                ...
                                messageCollection = messageCollection.concat(
                                    channelMessages.filter(msg => msg.author.id == user.id)
