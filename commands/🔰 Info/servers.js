...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
        ...
        try {
            ...
            var guilds_in = client.guilds.cache
                .sort((a, b) => b?.memberCount - a.memberCount)
                .map(
                    guild =>
                        `${handlemsg(client.la[ls].cmds.info.servers.map, { guildname: guild.name, guildid: guild.id, guildmemberCount: guild.memberCount })}\n`
                );
            swap_pages(
                client,
                message,
                guilds_in,
                `${handlemsg(client.la[ls].cmds.info.servers.title, { guildsin: guilds_in.length, bottag: client.user.tag })}`
            );
        } catch (e) {
