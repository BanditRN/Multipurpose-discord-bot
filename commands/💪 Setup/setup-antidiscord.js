        ...
        try {
            ...
            async function first_layer() {
                let menuoptions = [
                    {
                        ...
                        emoji: `${client.settings.get(message.guild.id, `antidiscord.enabled`) ? allEmojis.msg.ERROR : allEmojis.msg.SUCCESS}`,
                    },
                    {
                        value: "Settings",
                ...
                let MenuEmbed = new EmbedBuilder()
                    .setColor(es.color)
                    .setAuthor({ name: "Anti-Discord-Links System Setup", iconURL: "https://cdn.discordapp.com/emojis/858405056238714930.gif?v=1", url: "https://discord.gg/milrato" })
                ...
                collector.on("collect", menu => {
                    if (menu?.user.id === cmduser.id) {
                        ...
                        if (menu?.values[0] == "Cancel")
                            return menu?.reply({
                                content: eval(client.la[ls]["cmds"]["setup"]["setup-antidiscord"]["variable2"]),
                            });
                        menu?.deferUpdate();
                        used1 = true;
                    ...
                    case 0:
                        {
                            client.settings.set(
                                message.guild.id,
                                !client.settings.get(message.guild.id, `antidiscord.enabled`),
                                `antidiscord.enabled`
                            );
                                ...
                                .then(collected => {
                                    ...
                                    if (channel) {
                                        let antisettings = client.settings.get(
                                            message.guild.id,
                                            "antidiscord.whitelistedchannels"
                                        );
                            ...
                            tempmsg = await message.reply({
                                embeds: [
                                    new EmbedBuilder()
                                        ...
                                        .setColor(es.color)
                                        .setDescription(
                                            `Currently it is at: \`${client.settings.get(message.guild.id, "antidiscord.mute_amount")}\`\n\nPlease just send the Number! (0 means after the first time he/she will get muted)`
                                        )
                                        .setFooter(client.getFooter(es)),
                                ],
                            });
