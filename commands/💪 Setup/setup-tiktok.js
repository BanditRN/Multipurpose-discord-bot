...
*React with the Right Emoji according to the Right action*`
                ...
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `\`\`\` ${e.message ? e.message : e.stack ? String(e.stack).grey.substring(0, 2000) : String(e).grey.substring(0, 2000)}\`\`\``.substring(
                                    0,
                                    2000
                                )
                            )
            ...
            await tempmsg
                .awaitReactions({ filter, max: 1, time: 90000, errors: ["time"] })
                .then(collected => {
                    ...
                    .then(async collected => {
                        ...
                        if (msg && msg.mentions.channels.filter(ch => ch.guild.id == msg.guild.id).first()) {
                            client.social_log.set(
                                message.guild.id,
                                msg.mentions.channels.filter(ch => ch.guild.id == msg.guild.id).first().id,
                                "tiktok.dc_channel"
                            );
                            return message.reply({
                                embeds: [
...
https://www.tiktok.com/@milratodev`
                    ...
                    .then(async collected => {
                        ...
                        if (msg && msg.content) {
                            if (
                                ...
                                !msg.content.toLowerCase().includes("tiktok") &&
                ...
                await tempmsg
                    .awaitReactions({
                        filter: (reaction, user) => user.id == message.author.id && emojis.includes(reaction.emoji?.name),
                        max: 1,
                        time: 90000,
                        errors: ["time"],
                    })
                    .then(async collected => {
                        var channel = client.social_log.get(message.guild.id, "tiktok.channels")[
                            emojis.findIndex(emoji => emoji == collected.first().emoji?.name)
                        ];
