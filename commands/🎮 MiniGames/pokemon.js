...
class GuessThePokemon {
    constructor(options = {}) {
        ...
        if (typeof options.incorrectMessage !== "string")
            throw new TypeError("INCORRECT_MESSAGE: InCorrect Message must be a string.");

        if (!options.time) options.time = 60000;
        if (parseInt(options.time) < 10000)
            throw new TypeError("TIME_ERROR: Time cannot be less than 10 seconds in ms (i?.e 10000 ms)!");
        ...
        if (typeof options.thinkMessage !== "string")
            throw new TypeError("INVALID_THINK_MESSAGE: Think Message must be a string.");
    ...
    async startGame() {
        ...
        collector.on("collect", message => {
            ...
            if (!message.content || message.content.toLowerCase() === "stop") {
                return msg.edit({
                    content: this.options.stopMessage.replace("{pokemon}", data.name),
                    attachments: [],
                    embeds: [],
                });
            }
            ...
            return msg.edit({
                content: this.options.incorrectMessage.replace("{pokemon}", data.name),
                embeds: [],
                attachments: [],
            });
        });
