...
class TwoZeroFourEight {
    ...
    isBoardFull() {
        for (let y = 0; y < HEIGHT; y++) for (let x = 0; x < WIDTH; x++) if (this.board[y * WIDTH + x] === 0) return false;
        return true;
    }
        ...
        for (let y = 0; y < HEIGHT; y++) {
            for (let x = 0; x < WIDTH; x++) {
                ...
                ["down", "left", "right", "up"].forEach(dir => {
                    ...
                    if (
                        isInsideBlock(newPos, WIDTH, HEIGHT) &&
                        (this.board[newPos.y * WIDTH + newPos.x] === 0 || this.board[newPos.y * WIDTH + newPos.x] === posNum)
                    )
                        numMoves++;
                });
            }
    ...
    shift(pos, dir) {
        ...
        while (!set) {
            ...
            if (
                !isInsideBlock(moveTo, WIDTH, HEIGHT) ||
