...
class Connect4Game {
    constructor(options = {}) {
        ...
        if (typeof options.emojis.player1 !== "string")
            throw new TypeError("INVALID_EMOJI: Player1 Emoji must be a string.");
        if (!options.emojis.player2) options.emojis.player2 = "🟡";
        if (typeof options.emojis.player2 !== "string")
            throw new TypeError("INVALID_EMOJI: Player2 Emoji must be a string.");
    ...
    isBoardFull() {
        for (let y = 0; y < HEIGHT; y++)
            for (let x = 0; x < WIDTH; x++) if (this.gameBoard[y * WIDTH + x] === "⚪") return false;
        return true;
    }

    getResultText(result) {
        if (result.result === "tie") return this.options.drawMessage;
        else if (result.result === "timeout") return this.options.gameEndMessage;
        else if (result.result === "error") return "ERROR: " + result.error;
