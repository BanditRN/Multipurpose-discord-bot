...
const { format, delay, swap_pages, swap_pages2_interaction } = require(`${process.cwd()}/handlers/functions`);
                            ...
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    tracks
                                        .map(
                                            (track, i) =>
                                                `**${++i})** **${track.title.substring(0, 60)}** - \`${track.isStream ? `LIVE STREAM` : format(track.duration).split(` | `)[0]}\`\n*requested by: ${track.requester.tag}*`
                                        )
                                        .join(`\n`)
                                ),
                        ],
            ...
            for (let i = 0; i < tracks.length; i += 15) {
                ...
                quelist.push(
                    songs
                        .map(
                            (track, index) =>
                                `**${i + ++index})** **${track.title.substring(0, 60)}** - \`${track.isStream ? `LIVE STREAM` : format(track.duration).split(` | `)[0]}\`\n*requested by: ${track.requester.tag}*`
                        )
                        .join(`\n`)
                );
            }
