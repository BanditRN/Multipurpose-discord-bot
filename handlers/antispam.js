    ...
    async function checkAntiSpam(message) {
        try {
            ...
            try {
                ...
                if (!messagesmap.has(message.author.id)) messagesmap.set(message.author.id, []);
                ...
                if (
                    usermessages.filter(d => 10000 - (Date.now() - d) >= 0).length > messagelimit //filter messages of the last minute
                ) {
                        ...
                        message.channel.send({
                            embeds: [
                                new EmbedBuilder()
                                    ...
                                    .setDescription(
                                        `> <@${message.author.id}> **received an autogenerated Warn - \`antispam\`**!\n\n> **He now has \`${warnings.length} Warnings\`**`
                                    ),
                            ],
                        });
