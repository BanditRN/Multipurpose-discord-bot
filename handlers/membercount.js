        ...
        async function Channel(chId, channelName) {
            ...
            try {
                ...
                let newname = String(channelName)
                    ...
                    .replace(
                        /{online}/i,
                        guild.members.cache.filter(member => member.presence && member.presence.status == "online").size
                    )
                    .replace(/{offline}/i, guild.members.cache.filter(member => member.presence).size)
                    .replace(
                        /{idle}/i,
                        guild.members.cache.filter(member => member.presence && member.presence.status == "idle").size
                    )
                    .replace(
                        /{dnd}/i,
                        guild.members.cache.filter(member => member.presence && member.presence.status == "dnd").size
                    )
                    ...
                    .replace(
                        /{offlinemember}/i,
                        guild.members.cache.filter(member => !member.user.bot && !member.presence).size
                    )
                    .replace(
                        /{idlemember}/i,
                        guild.members.cache.filter(
                            member => !member.user.bot && member.presence && member.presence.status == "idle"
                        ).size
                    )
                    .replace(
                        /{dndmember}/i,
                        guild.members.cache.filter(
                            member => !member.user.bot && member.presence && member.presence.status == "dnd"
                        ).size
                    )
                    .replace(
                        /{allonlinemember}/i,
