...
module.exports = client => {
    ...
    client.JobTimesMessages = new CronJob(
        "10 * * * * *",
        function () {
    ...
    async function timedmessage(guildid) {
        try {
            ...
            let timedmessages_to_send = timedmessages.filter(
                msg =>
                    msg.days.includes(currentDay) &&
                    Number(msg.minute) == Number(currentMinute) &&
                    Number(msg.hour) == Number(currentHour)
            ...
            if (timedmessages_to_send.length > 0) {
                for (const msg of timedmessages_to_send) {
                    let channel =
                        guild.channels.cache.get(msg.channel) ||
                        (await guild.channels.fetch(msg.channel).catch(() => {})) ||
                        false;
                    if (!channel) return;
                    if (msg.embed) {
