         ...
         */
            ...
            if (!user || user == null || !user.id) {
                alluser = message.guild.members.cache.map(member =>
                    String(member.displayName + "#" + member.user.discriminator).toLowerCase()
                );
                user = alluser.find(user => user.startsWith(args.join(" ").toLowerCase()));
                user = message.guild.members.cache.find(
                    me => String(me.displayName + "#" + me.user.discriminator).toLowerCase() == user
                );
...
async function edit_Roster_msg(client, guild, the_roster_db, pre) {
    try {
        ...
        if (data.rosterchannel == "notvalid") return; //console.log("Roster Channel not valid | :: | " + data.rosterchannel);
        ...
        if (!channel) channel = client.channels.cache.get(data.rosterchannel);
        //if the rosterchannel is not found, then send error + return
        if (!channel) return; //console.log("Roster Channel not found | :: | " + data.rosterchannel);
        //if the defined message length is less then 2 try return error (not setupped)
        if (data.rostermessage.length < 5) return; //console.log("Roster Message not valid | :: | " + data.rostermessage);
        //fetch the message from the channel
        let message =
            channel.messages.cache.get(data.rostermessage) ||
            (await channel.messages.fetch(data.rostermessage).catch(() => {})) ||
            false;
        ...
        for (let j = 0; j < rosterroles.length; j++) {
            ...
            if (data.rosterstyle == "1") {
                ...
                let memberarray = role.members.map(
                    member =>
                        `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} <@${member.user.id}> | \`${member.user.tag}\``
                );
                ...
                //define the memberarray
                let memberarray = role.members.map(
                    member => `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} <@${member.user.id}>`
                );
                //loopthrough the array for 20 members / page
                for (let i = 0; i < memberarray.length; i += 20) {
                    var thearray = memberarray;
                ...
                //define the memberarray
                let memberarray = role.members.map(
                    member => `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} **${member.user.tag}**`
                );
                //loopthrough the array for 20 members / page
                for (let i = 0; i < memberarray.length; i += 20) {
                    var thearray = memberarray;
                ...
                //define the memberarray
                let memberarray = role.members.map(
                    member => `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} **${member.user.username}**`
                );
                //loopthrough the array for 20 members / page
                for (let i = 0; i < memberarray.length; i += 20) {
                    var thearray = memberarray;
                ...
                let memberarray = role.members.map(
                    member =>
                        `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} <@${member.user.id}> | \`${member.user.id}\``
                );
                //loopthrough the array for 20 members / page
                for (let i = 0; i < memberarray.length; i += 20) {
                    var thearray = memberarray;
                ...
                let memberarray = role.members.map(
                    member =>
                        `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} <@${member.user.id}> | **${member.user.username}**`
                );
                //loopthrough the array for 20 members / page
                for (let i = 0; i < memberarray.length; i += 20) {
                    var thearray = memberarray;
            ...
            if (totalbreak)
                return message.edit({ embeds: [rosterembed] }).catch(e => console.log("could not edit roster 1" + e));
        }
...
async function send_roster_msg(client, guild, the_roster_db, pre) {
    ...
    for (let i = 0; i < rosterroles.length; i++) {
        ...
        //define the memberarray
        let memberarray = role.members.map(
            member => `${the_roster_db?.get(guild.id, pre + ".rosteremoji")} <@${member.user.id}> | \`${member.user.tag}\``
        );
        //loopthrough the array for 20 members / page
        for (let i = 0; i < memberarray.length; i += 20) {
            var thearray = memberarray;
...
async function create_transcript_buffer(Messages, Channel, Guild) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await msgs.forEach(async msg => {
                ...
                if (msg.type == "DEFAULT") {
                    ...
                    if (msg.reactions && msg.reactions.cache.size > 0) {
                        ...
                        for (const reaction of msg.reactions.cache.map(
                            this_Code_is_by_Tomato_6966 => this_Code_is_by_Tomato_6966
                        )) {
            ...
            baseHTML +=
                `<body><div class="info"><div class="guild-icon-container"> <img class="guild-icon" src="${Guild.iconURL({ dynamic: true })}" />` +
...
function getMember(message, toFind = "") {
    ...
    if (!target && message.mentions.members)
        target = message.mentions.members.filter(member => member.guild.id == message.guild.id).first();
    if (!target && toFind) {
...
function stations(client, prefix, message) {
    ...
    try {
        ...
        const stationsembed4 = new EmbedBuilder()
            ...
            .setFooter(client.getFooter(es))
            .setTitle("CUSTOM REQUESTS | Pick your Station, by typing in the right `INDEX` Number!");
            ...
            let embed = new EmbedBuilder()
                ...
                .setDescription(
                    config.settings.LeaveOnEmpty_Queue.enabled && type != "skip"
                        ? `I'll leave the Channel: \`${client.channels.cache.get(player.voiceChannel).name}\` in: \`${ms(config.settings.LeaveOnEmpty_Queue.time_delay, { long: true })}\`, If the Queue stays Empty! `
                        : eval(client.la[ls]["handlers"]["functionsjs"]["functions"]["variable9"])
                )
                        ...
                        if (player.queue.size === 0) {
                            ...
                            try {
                                embed.setDescription(
                                    eval(client.la[ls]["handlers"]["functionsjs"]["functions"]["variable1"])
                                );
                            } catch {}
                            try {
...
async function swap_pages(client, message, description, TITLE) {
    ...
    collector.on("collect", async b => {
        if (b?.user.id !== message.author.id)
            return b?.reply({
                content: `<:no:833101993668771842> **Only the one who typed ${prefix}help is allowed to react!**`,
                ephemeral: true,
            });
        //page forward
        if (b?.customId == "1") {
            ...
            if (currentPage !== 0) {
                currentPage -= 1;
                await swapmsg
                    .edit({ embeds: [embeds[currentPage]], components: getDisabledComponents[swapmsg.components] })
                    .catch(() => {});
                await b?.deferUpdate();
            } else {
        ...
        if (guildid) {
            ...
            dbEnsure(client.settings, guildid, {
                ...
                embed: {
                    ...
                    footericon: client.guilds.cache.get(guildid)
                        ? client.guilds.cache.get(guildid).iconURL({
                              dynamic: true,
                          })
                        : ee.footericon,
                },
...
function change_status(client) {
    try {
        client.user.setActivity(
            `${config.prefix}help | ${config.prefix}setup | ${totalGuilds} Guilds | ${Math.ceil(totalMembers / 1000)}k Members`,
            {
                type: "WATCHING",
                shardID: shard,
            }
        ...
        client.user.setActivity(
            `${config.prefix}help | ${config.prefix}setup | ${client.guilds.cache.size} Guilds | ${Math.ceil(client.users.cache.size / 1000)}k Members`,
            {
                type: "WATCHING",
                shardID: 0,
            }
                            ...
                            if (client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id) {
                                ...
                                if (!members || members == undefined || members.length == undefined || members.length == 0) {
                                    ...
                                    if (vc.permissionsFor(vc.guild.members.me).has(PermissionFlagsBits.ManageChannels)) {
                                        ...
                                        console.log(
                                            `I couldn't delete the Channel: ${vc.name} in: ${vc.guild ? vc.guild.name : "undefined"} DUE TO EMPTYNESS`
                                                .strikethrough.brightRed
                                        );
                                    }
                                }
...
function create_join_to_create_Channel(client, voiceState, type) {
    ...
    //Create the channel
    voiceState.guild.channels
        .create(String(chname.replace("{user}", voiceState.member.user.username)).substring(0, 32), createOptions)
        .then(async vc => {
            console.log(
                `Created the Channel: ${String(chname.replace("{user}", voiceState.member.user.username)).substring(0, 32)} in: ${voiceState.guild ? voiceState.guild.name : "undefined"} after: ${Date.now() - DateNow}ms`
                    .brightGreen
            );
            //add to the DB
        ...
        if (guildid) {
            ...
            dbEnsure(client.settings, guildid, {
                ...
                embed: {
                    ...
                    footericon: client.guilds.cache.has(guildid)
                        ? client.guilds.cache.get(guildid).iconURL({
                              dynamic: true,
                          })
                        : ee.footericon,
...
function ensure_economy_user(client, guildid, userid) {
    ...
    if (data.black_market.boost.time !== 0 && 86400000 * 2 - (Date.now() - data.black_market.boost.time) <= 0) {
        console.log(
            `Reset Multiplier from Black Market for: ${userid} | TIME: ${86400000 * 2 - (Date.now() - data.black_market.boost.time)}`
        );
    ...
    return new Promise(async (res, rej) => {
        try {
            ...
            if (typeof ChannelLink !== "string")
                return rej(`Passed in ${typeof ChannelLink} but a String would be required for the ChannelLink`);
            ...
            content = content.items.map(v => {
                ...
                OBJ.author = v.author;
                ((OBJ.id = v.link.split("watch?v=")[1] || v.id), (OBJ.isoDate = v.isoDate));
                return OBJ;
            });
...
const channelInfo = (url, options = {}) =>
    __awaiter(void 0, void 0, void 0, function* () {
        var _a,
            ...
            _32,
            _33;
        if (typeof url !== "string") throw new Error(`Expected "url" to be "string" but received "${typeof url}".`);
        ...
        try {
            initialData = JSON.parse(
                (_a = res.split("var ytInitialData = ")[1]) === null || _a === void 0 ? void 0 : _a.split(";</script>")[0]
            );
        } catch (err) {
        ...
        const channel = {
            name:
                initialData && initialData.metadata && initialData.metadata.channelMetadataRenderer
            ...
            subscribers:
                ...
                initialData.header.c4TabbedHeaderRenderer.subscriberCountText
                    ? initialData.header.c4TabbedHeaderRenderer.subscriberCountText.simpleText
                    : undefined,
            banner:
                initialData &&
                initialData.header &&
                ...
                initialData.header.c4TabbedHeaderRenderer.banner.thumbnails
                    ? initialData.header.c4TabbedHeaderRenderer.banner.thumbnails.sort((a, b) => b?.width - a.width)
                    : undefined,
            tvBanner:
                initialData &&
                initialData.header &&
                ...
                initialData.header.c4TabbedHeaderRenderer.tvBanner.thumbnails
                    ? initialData.header.c4TabbedHeaderRenderer.tvBanner.thumbnails.sort((a, b) => b?.width - a.width)
                    : undefined,
            mobileBanner:
                initialData &&
                initialData.header &&
                ...
                initialData.header.c4TabbedHeaderRenderer.mobileBanner.thumbnails
                    ? initialData.header.c4TabbedHeaderRenderer.mobileBanner.thumbnails.sort((a, b) => b?.width - a.width)
                    : undefined,
            tags:
                initialData && initialData.metadata && initialData.metadata.channelMetadataRenderer
            ...
            unlisted:
                initialData && initialData.microformat && initialData.microformat.microformatDataRenderer
...
async function isValidSnowflakeId(input) {
    const SNOWFLAKE_REGEX = /^(?<id>\d{17,20})$/;
    return typeof input === "string" && SNOWFLAKE_REGEX.test(input);
}
