        ...
        if (oldState.channelId) {
            if (vcmessages.map(d => d.channelId).includes(oldState.channelId)) {
                ...
                if (theData) {
                    ...
                    if (!channel)
                        channel = (await oldState.guild.channels.fetch(theData.textChannelId).catch(() => {})) || false;
        ...
        if (newState.channelId) {
            if (vcmessages.map(d => d.channelId).includes(newState.channelId)) {
                ...
                if (theData) {
                    ...
                    if (!channel)
                        channel = (await newState.guild.channels.fetch(theData.textChannelId).catch(() => {})) || false;
                    if (!channel) return; // console.log("Channel got deleted / unable to find the channel in this guild..")
                    ...
                    channel
                        .send({
                            ...
                            content:
                                `${theData.message && theData.message.length > 10 ? theData.message : "*No __VALID__ Message Added*"}`.substring(
                                    0,
                                    2000
                                ),
                        })
