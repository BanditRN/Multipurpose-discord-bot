...
async function search(client, message, args, type, slashCommand) {
    ...
    try {
        ...
        try {
            collected = await toreact.awaitReactions({ filter, max: 1, time: 30e3, errors: ["time"] });
        } catch (e) {
        ...
        var pickedresults = theresults
            .map(
                (track, ii) =>
                    `${index == ii ? "" : "~~"}**${++ii})** [\`${String(track.title).substring(0, 60).split("[").join("{").split("]").join("}")}\`](${track.uri}) - \`${format(track.duration).split(" | ")[0]}\`${index == ii ? "" : "~~"}`
            )
            .join("\n");
