...
module.exports = (client, preindex) => {
    ...
    client.on("interactionCreate", async interaction => {
        ...
        let channelname = ticket.defaultname
         ...
         */
        optionsData.permissionOverwrites = [...guild.roles.cache.filter(d => d.id != guild.id).values()]
                ...
                if (ch.permissionsFor(ch.guild.members.me).has(PermissionFlagsBits.SendMessages)) {
                    if (ch.permissionsFor(ch.guild.members.me).has(PermissionFlagsBits.EmbedLinks)) {
                        ...
                        await ch
                            .send({
                                content:
                                    `<@${user.id}> ${ticketroles.length > 0 ? "| " + ticketroles.join(" / ") : ""}\n${ticketembeds[0].description}`.substring(
                                        0,
                                        2000
                                    ),
                                components: allbuttons,
