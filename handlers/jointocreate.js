...
const { databasing, check_voice_channels, check_created_voice_channels, create_join_to_create_Channel } = require(
    `./functions`
);
...
module.exports = function (client) {
    ...
    client.JobJointocreate = new CronJob(
        "0 * * * * *",
        function () {
    ...
    client.JobJointocreate2 = new CronJob(
        "0 * * * * *",
        function () {
    ...
    client.on("voiceStateUpdate", (oldState, newState) => {
        ...
        if (!oldState.channelId && newState.channelId) {
            ...
            if (!index) {
                for (let i = 1; i <= maxJoinToCreate; i++) {
                    ...
                    if (
                        d?.has(newState.guild.id) &&
                        d?.has(newState.guild.id, pre) &&
                        d?.get(newState.guild.id, pre + ".channel").includes(newState.channelId)
                    )
                        index = i;
                }
            }
        ...
        if (oldState.channelId && !newState.channelId) {
            if (
                client.jointocreatemap.has(`tempvoicechannel_${oldState.guild.id}_${oldState.channelId}`) &&
                oldState.guild.channels.cache.has(
                    client.jointocreatemap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelId}`)
                )
            ) {
