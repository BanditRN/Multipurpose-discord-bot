    ...
    client.on("messageCreate", message => {
        ...
        if (message.channel.id == client.settings.get(message.guild.id, `counter`)) {
            ...
            if (!message.author.bot && message.author.id === counterauthor) {
                ...
                message.reply(eval(client.la[ls]["handlers"]["counterjs"]["counter"]["variable1"])).then(m =>
                    setTimeout(() => {
                        m.delete();
                    }, 3000)
                );
            ...
            if (!message.author.bot && isNaN(message.content)) {
                ...
                message.reply(eval(client.la[ls]["handlers"]["counterjs"]["counter"]["variable2"])).then(m =>
                    setTimeout(() => {
                        m.delete();
                    }, 3000)
                );
            ...
            if (!message.author.bot && parseInt(message.content) !== count + 1) {
                ...
                message.reply(eval(client.la[ls]["handlers"]["counterjs"]["counter"]["variable3"])).then(m =>
                    setTimeout(() => {
                        m.delete();
                    }, 3000)
                );
