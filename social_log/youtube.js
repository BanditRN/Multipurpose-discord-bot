...
module.exports = client => {
    ...
    async function checkVideos(youtubeChannel, youtuber) {
        try {
            ...
            if (lastSavedVideo && (lastSavedVideo === lastVideos[0].id || lastSavedVideo.includes(lastVideos[0].id)))
                return false;
            ...
            .forEach(guildid => {
                try {
                    ...
                    client.social_log.get(guildid, "youtube.channels").forEach(async youtuber => {
                        ...
                        if (!channelInfos)
                            return console.log(
                                String(" [YOUTUB] :: [ERR] | Invalid youtuber provided: " + youtuber).italic.brightRed
                            );
                        //channelInfos.url, ChannelDATA
                        let video = await checkVideos(channelInfos.url, youtuber);
                        if (!video)
                            return console.log(` [YOUTUB] :: [${channelInfos.name}] | No notification`.italic.brightRed);

                        let channel = await client.channels
                            .fetch(client.social_log.get(guildid, "youtube.dc_channel"))
                            .catch(e => {
                                return;
                            });
                        ...
                        channel
                            .send({
                                content: client.youtube_log
                                    .get(youtuber, "message")
