...
module.exports = client => {
    ...
    client.Jobtwitterfeed = new CronJob(
        "0 */7 * * * *",
        async function () {
...
async function create_twit(client) {
    ...
    var userids = client.social_log
        .filterArray(value => value?.twitter?.TWITTER_USER_NAME_ONLY_THOSE?.length > 2)
        .map(value => `${value.twitter.TWITTER_USER_NAME_ONLY_THOSE}`);
            ...
            await T.get("search/tweets", { q: `from:${user}`, count: 10 }, function (err, data, response) {
                try {
                    ...
                    if (
                        ...
                        !data.statuses[0] ||
                        data.statuses[0] == undefined ||
                        !data.statuses[0].user
                    )
                        return console.log("COULD NOT FIND THE DATA".cyan);
                    ...
                    if (!guildid || guildid == null || guildid == undefined || guildid.length != 18)
                        return console.log(" [TWITER] :: NO VALID GUILD ".cyan);
                    try {
                    ...
                    if (String(twitsettings.TWITTER_USER_NAME_ONLY_THOSE).toLowerCase() != String(TwitterName).toLowerCase())
                        return;
                    ...
                    if (
                        ...
                        client.channels
                            .fetch(twitsettings.DISCORD_CHANNEL_ID)
                            .then(channel => {
                                channel
                                    .send(twitsettings.infomsg.replace("{Twittername}", TwitterName).replace("{url}", url))
                                    .then(msg => {
                                        console.log(
                                            ` [TWITER] :: NOTIFICATION SENT IN ${channel.name} for ${TwitterName} with ${url}`
                                                .green
                                        );
